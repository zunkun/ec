module.exports = {
	btrip: {
		flight: 'dingtalk.oapi.alitrip.btrip.flight.order.search',
		train: 'dingtalk.oapi.alitrip.btrip.train.order.search',
		vehicle: 'dingtalk.oapi.alitrip.btrip.vehicle.order.search',
		hotel: 'dingtalk.oapi.alitrip.btrip.hotel.order.search',
		approvalNew: 'dingtalk.oapi.alitrip.btrip.approval.new',
		approvalUpdate: 'dingtalk.oapi.alitrip.btrip.approval.update'
	}
};
